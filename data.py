...
def set_up_data_s2s(resolution=512, frame_mode="reconstruct", sequence_length=16, mpi_size=1, rank=0, n_batch=1, num_images_visualize=1):
    train_dataset = VideoDataset('/shared/ademi_adeniji/something-something/drawers', sequence_length,
                        train=True, resolution=resolution, frame_mode=frame_mode)
    val_dataset = VideoDataset('/shared/ademi_adeniji/something-something/drawers', sequence_length,
                        train=False, resolution=resolution, frame_mode=frame_mode)
    train_sampler = data.DistributedSampler(train_dataset, num_replicas=mpi_size, rank=rank)
    train_dataloader = data.DataLoader(
        train_dataset,
        batch_size=n_batch // mpi_size,
        num_workers=4,
        pin_memory=True,
        shuffle=False,
        sampler=train_sampler,
    )
    val_sampler = data.DistributedSampler(val_dataset, num_replicas=mpi_size, rank=rank)
    ...
    viz_sampler = data.DistributedSampler(val_dataset, num_replicas=mpi_size, rank=rank)
    viz_dataloader = data.DataLoader(
        val_dataset,
        batch_size=num_images_visualize // mpi_size,
        num_workers=4,
        pin_memory=True,
        sampler=viz_sampler,
    )
    ...
    image_channels = 3
    return train_dataloader, val_dataloader, viz_dataloader

def set_up_data(dataset, data_root, test_eval):
    shift_loss = -127.5
    scale_loss = 1. / 127.5
    if dataset == 'imagenet32':
        trX, vaX, teX = imagenet32(data_root)
        image_size = 32
        image_channels = 3
        shift = -116.2373
        scale = 1. / 69.37404
    elif dataset == 'imagenet64':
        trX, vaX, teX = imagenet64(data_root)
        image_size = 64
        image_channels = 3
        shift = -115.92961967
        scale = 1. / 69.37404
    elif dataset == 'ffhq_256':
        trX, vaX, teX = ffhq256(data_root)
        image_size = 256
        image_channels = 3
        shift = -112.8666757481
        scale = 1. / 69.84780273
    elif dataset == 'ffhq_1024':
        trX, vaX, teX = ffhq1024(data_root)
        image_size = 1024
        image_channels = 3
        shift = -0.4387
        scale = 1.0 / 0.2743
        ...
        (trX, _), (vaX, _), (teX, _) = cifar10(data_root, one_hot=False)
        image_size = 32
        image_channels = 3
        shift = -120.63838
        scale = 1. / 64.16736
    ...
    do_low_bit = dataset in ['ffhq_256']
    ...
    return train_data, valid_data, preprocess_func
